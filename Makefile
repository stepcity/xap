...
kdedir=/opt/kde
gnomedir=/opt/gnome
mlvwmdir=/usr/local/X11
libdir=/usr/local/lib/xap
...
CC		= gcc
LIBS	= -lgtk -lgdk -lglib -lX11 -L/usr/local/lib -L$(prefix)/lib -L/usr/lib -L/usr/lib  -L/usr/lib64
CFLAGS	= -g -O2 -I$(prefix)/include  -DTERMINAL=\"/usr/bin/X11/xterm\" -DURLFETCH="\"/usr/local/bin/curl\"" -DGVIM="\"/usr/bin/gvim\"" -DADOUBLE -DHAVE_CONFIG_H  -I/usr/include -I/usr/lib/glib/include -D_REENTRANT -DVERSION=\"$(VERSION)\" \
		-DINSTDIR=\"$(prefix)\" -DICONDIR=\"${icondir}\" \
		-DLOCALEDIR=\"${datadir}/locale\" -I. -I./lib-src \
		-DPLUGINDIR=\"$(libdir)\" -DDATA_DIR=\"${datadir}\" -DXAP_RC=\"xap.rc\" \
		-I/usr/include/glib-1.2 -I/usr/include/gtk-1.2 -I/usr/lib64/glib/include
